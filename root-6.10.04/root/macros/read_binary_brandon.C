...
void decode(char *filename) {
   ...
   double waveform[16][4][1024], events[16][4][1024], time[16][4][1024], amplitude[16][4][1024];
   float bin_width[16][4][1024];
   int i, j, k, b, chn, n, chn_index, n_boards;
   double t1, t2, e1, dt;
   //char filename[256];
   char rootfile[256];
   int ndt, a[4];
   double threshold, sumdt, sumdt2;
   double sum, baseline, max[4], a1, a2, a3, a4;
   ...
   rec->Branch("a1", amplitude[0][0],"a1[1024]/D");
   rec->Branch("a2", amplitude[0][1],"a2[1024]/D");
   rec->Branch("a3", amplitude[0][2],"a3[1024]/D");
   rec->Branch("a4", amplitude[0][3],"a4[1024]/D");
   rec->Branch("e1", events[0][0]   ,"e1[1024]/D");
   rec->Branch("e2", events[0][1]   ,"e2[1024]/D");
   rec->Branch("e3", events[0][2]   ,"e3[1024]/D");
   rec->Branch("e4", events[0][3]   ,"e4[1024]/D");
   // create canvas
   TCanvas *c1 = new TCanvas();
   
   // create graph
   TGraph *g = new TGraph(1024, (double *)time[0][0], (double *)amplitude[0][0]);
   ...
   for (n=0 ; ; n++) {
      ...
      for (b=0 ; b<n_boards ; b++) {
         ...
         for (chn=0 ; chn<4 ; chn++) {
            ...
            for (i=0 ; i<1024 ; i++) {
               ...
               amplitude[0][chn_index][i] = (voltage[i] / 65536. + eh.range/1000.0 - 0.5);
     ...
     sum=0.0;
      for (i=0 ; i<10; i++) {
       ...
       baseline=sum/10;
     //Find amplitudes for all channels
      for (k=0 ; k<4 ; k++) {
         max[k]=-10000.0;
         for (i=0 ; i<1022; i++) {
            if (waveform[b][k][i]>max[k]) {
               max[k]=waveform[b][k][i];
